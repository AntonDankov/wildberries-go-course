<script lang="ts">
	...
	async function confirmBooking(bookId: number) {
		confirmingId = bookId;
		confirmError = null;
		try {
			const res = await fetch(`http://localhost:8080/events/${bookId}/confirm`, {
				method: 'POST'
			});
			if (!res.ok) {
				const err = await res.json().catch(() => ({}));
	...
	<div class="mx-auto max-w-2xl">
		...
		{:else if event}
			<section class="bg-card border-border mb-8 flex flex-col gap-4 rounded-xl border p-6">
				...
				<Button class="self-end rounded-full" disabled={booking} onclick={bookEvent}>
					{#if booking}<LoaderCircle size={14} class="animate-spin" />{:else}<BookOpen
							size={14}
						/>{/if}
			...
			{:else}
				<ul class="flex flex-col gap-2.5">
					{#each books as book (book.id)}
						...
						<li
							class="bg-card border-border flex items-center justify-between gap-3 rounded-xl border px-5 py-4"
						>
							<div class="flex flex-col gap-1">
							...
							<div class="flex shrink-0 items-center gap-2">
								{#if book.book_status === 0}
									...
									>
										...
										{/if}
										{confirmingId === book.id ? 'Confirming…' : 'Confirm'}
									</Button>
								{/if}
								<Badge variant="outline" class="text-xs {status.class}">
									{status.label}
								</Badge>
							</div>
						</li>
